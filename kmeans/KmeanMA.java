...
public class KmeanMA {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException{
	    	...
	    	for(i=0 ; i<clusternum; i++) {
	    		...
	    		for(int j=0; j<DIMS; j++) {	    			
	    			dist += Math.abs((data[j] - center[i][j]));
	    	...
	    	context.write(new LongWritable(-1), new Text(String.valueOf(min)));
	...
	public static void main(String[] args) throws Exception {
	    ...
	    //conf.set("clusterInPath", "inputc2");
	    conf.set("clusterInPath", args[2]);
	    long startTime = System.currentTimeMillis();
	    Job job = new Job(conf, "word count");
	    job.setJarByClass(KmeanMA.class);
	    ...
	    for(i=1;i<iter;i++){
			...
			job2.setJarByClass(KmeanMA.class);
